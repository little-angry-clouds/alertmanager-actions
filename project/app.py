...
class AlertmanagerActions:
    ...
    def launch_action(self):
        ...
        if not request.content_type:
            self.logger.warning("The received content type should be 'application/json'.")
    ...
    def _treat_action(self, action_name, treated_actions):
        ...
        if action_name in treated_actions:
            self.logger.debug("Action already treated, so the command won't be executed")
    ...
    def _lock_action(self, action_name):
        ...
        if self.lock[action_name]:
            self.logger.debug("The lock for '%s' is active, so the command won't be executed" % action_name)
            return True
